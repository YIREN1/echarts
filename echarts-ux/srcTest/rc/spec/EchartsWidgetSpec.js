...
define(['baja!',
        'jquery',
        'Promise',
        'nmodule/echarts/rc/EchartsWidget',
  ...
  describe('nmodule/echarts/rc/EchartsWidget', function () {
    var stooges,
        widget,
        elem;
    ...
    describe('#doInitialize()', function () {
      it('creates the structure to load buttons into', function () {
        expect(elem.find('.EchartsWidget-header').length).toBe(1);
        expect(elem.find('.EchartsWidget-content').length).toBe(1);
      });

      it('arms a handler to set active class on button click', function () {
        var contentDom = elem.find('.EchartsWidget-content'),
      ...
      it('arms a handler to display selected slot name', function () {
        var slotDom = elem.find('.EchartsWidget-selected-slot');

        runs(function () {
          var contentDom = elem.find('.EchartsWidget-content'),
              button = $('<button class="EchartsWidget-button" data-slot="curlyJoe"></button>');
